            ...
            with self._lock_registry():
                ...
                if status.status == 'processing' and status.instance_id != self.instance_id:
                    ...
                    if datetime.now() - started_at < timedelta(hours=1):  # 1 hour timeout
                        ...
                        logger.warning(f"File {file_path} processing by {status.instance_id} timed out, allowing reprocessing")
                        return True
    ...
    def acquire_file_lock(self, file_path: Path) -> bool:
        ...
        try:
            ...
            with self._lock_registry():
                ...
                retry_count = current_status.retry_count + 1 if current_status and current_status.status == 'failed' else 0
