...
from core.models import DocumentChunk, EmbeddingBatch, EmbeddingStats, ProcessingResult
...
class TestDocumentProcessing:
    ...
    def test_process_documents_with_threading(self, mock_executor, test_data_dir):
        ...
        mock_future.result.return_value = [DocumentChunk.create("test content", str(test_data_dir / "test.txt"))]
...
class TestEndToEndWorkflows:
    ...
    def test_complete_text_processing_workflow(self, test_data_dir):
        ...
        config = EmbeddingConfig(
            doctype="txt", chunking_strategy="fixed", chunk_size=30, batch_size_embeddings=2, vector_db_type="faiss"
        )
        service = DocumentService(config)
    ...
    def test_complete_api_processing_workflow(self, test_data_dir):
        ...
        config = EmbeddingConfig(doctype="api", batch_size_embeddings=1)
        service = DocumentService(config)
    ...
    def test_different_document_types_workflow(self, test_data_dir):
        ...
        for doctype in document_types:
            config = EmbeddingConfig(doctype=doctype, chunking_strategy="fixed", chunk_size=40)
            service = DocumentService(config)
    ...
    def test_chunking_strategies_comparison(self, test_data_dir):
        ...
        for strategy in strategies:
            config = EmbeddingConfig(doctype="txt", chunking_strategy=strategy, chunk_size=30)
            service = DocumentService(config)
