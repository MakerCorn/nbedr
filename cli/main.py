...
def add_create_embedding_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--datapath", type=Path, 
                        help="Path to the input document or directory (for local sources)")
    parser.add_argument("--output", type=str, default="./embeddings_output",
                        help="Path to save the generated embeddings (default: ./embeddings_output)")
    
    # Input Source Arguments
    parser.add_argument("--source-type", type=str, default="local",
                        choices=["local", "s3", "sharepoint"],
                        help="Type of input source (default: local)")
    parser.add_argument("--source-uri", type=str,
                        help="URI for the input source (S3 bucket, SharePoint URL, or local path)")
    parser.add_argument("--source-credentials", type=str,
                        help="JSON string containing credentials for the input source")
    parser.add_argument("--source-include-patterns", type=str,
                        help="JSON array of glob patterns to include (default: ['**/*'])")
    parser.add_argument("--source-exclude-patterns", type=str,
                        help="JSON array of glob patterns to exclude")
    parser.add_argument("--source-max-file-size", type=int, default=50*1024*1024,
                        help="Maximum file size in bytes (default: 50MB)")
    parser.add_argument("--source-batch-size", type=int, default=100,
                        help="Batch size for processing source files (default: 100)")

    # Document Processing Arguments
    parser.add_argument("--chunk-size", type=int, default=512,
                        help="Size of each chunk in tokens (default: 512)")
    ...
    parser.add_argument("--batch-size-embeddings", type=int, default=100,
                        help="Batch size for embedding generation (default: 100)")

    # Vector Database Configuration
    parser.add_argument("--vector-db-type", type=str, default="faiss",
                        choices=["faiss", "pinecone", "chroma"],
                        help="Vector database type (default: faiss)")
    parser.add_argument("--vector-db-config", type=str,
                        help="JSON string of vector database configuration")
    ...
    parser.add_argument("--pinecone-index-name", type=str, default="rag-embeddings",
    ...
    parser.add_argument("--chroma-collection-name", type=str, default="rag-embeddings",
                        help="Chroma collection name (default: rag-embeddings)")
    
    # FAISS specific
    parser.add_argument("--faiss-index-path", type=str, default="./faiss_index",
                        help="FAISS index path (default: ./faiss_index)")
    ...
    parser.add_argument("--workers", type=int, default=1,
                        help="Number of worker threads for processing (default: 1)")
    parser.add_argument("--embed-workers", type=int, default=1,
                        help="Number of worker threads for embedding generation (default: 1)")
    parser.add_argument("--pace", action="store_true", default=True,
                        help="Pace API calls to stay within rate limits (default: True)")

    # Rate Limiting Arguments
    parser.add_argument("--rate-limit", action="store_true",
                        help="Enable rate limiting for API requests")
    parser.add_argument("--rate-limit-strategy", type=str, 
                        choices=["fixed_window", "sliding_window", "token_bucket", "adaptive"],
                        default="sliding_window",
                        help="Rate limiting strategy (default: sliding_window)")
    parser.add_argument("--rate-limit-preset", type=str,
                        choices=["openai_gpt4", "openai_gpt35_turbo", "azure_openai_standard", 
                                "anthropic_claude", "conservative", "aggressive"],
                        help="Use a preset rate limit configuration")
    parser.add_argument("--rate-limit-requests-per-minute", type=int,
                        help="Maximum requests per minute")
    parser.add_argument("--rate-limit-tokens-per-minute", type=int,
                        help="Maximum tokens per minute")
    parser.add_argument("--rate-limit-max-burst", type=int,
...
def add_search_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--index-path", type=str,
                        help="Path to vector index (for FAISS)")
...
def add_list_sources_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--detailed", action="store_true",
                        help="Show detailed information about each source")
...
def add_status_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--check-connections", action="store_true",
                        help="Check connections to external services")
